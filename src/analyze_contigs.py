...
def plot_contig_motif_heatmap(contigs: list[Contig]):
    ...
    for contig in contigs:
        for motif in contig.motifs:            
            ...
            if i == 2:
                ...
                def ks_permutation_test(data1, data2, n_permutations=10000, ci=95):
                    """
                    ...
                    # Store the KS statistics from permutations
                    permutation_ks_values = np.empty(n_permutations)

                    for i in range(n_permutations):
                        # Permute the combined data
                        permuted_data = np.random.permutation(combined_data)
                        # Split into two samples
                        permuted_sample1 = permuted_data[:n1]
                        permuted_sample2 = permuted_data[n1:]

                        # Calculate KS statistic for the permuted samples
                        permutation_ks_values[i] = stats.ks_2samp(permuted_sample1, permuted_sample2).statistic

                    # Calculate the empirical p-value
                    # Count how many permuted statistics are >= the observed statistic
                    # The KS statistic (D) is always non-negative. A larger value means more difference.
                    # We want the probability of observing a KS statistic as large or larger than
                    # the observed one, under the null hypothesis.
                    p_value = np.mean(permutation_ks_values >= observed_ks_value)
                    lower = np.percentile(permutation_ks_values, (100-ci)/2)
                    upper = np.percentile(permutation_ks_values, 100 - (100-ci)/2)

                    return p_value, lower, upper

                # Perform bootstrap tests instead of KS tests
                means_sig, _, _ = ks_permutation_test(group1_data, group2_data)
                se_sig, _, _ = ks_permutation_test(g1_se, g2_se) # Comparing distribution of standard errors

                # Promoter regions
                means_sig_pr, _, _ = ks_permutation_test(group1_data_pr, group2_data_pr)
                se_sig_pr, _, _ = ks_permutation_test(g1_se_pr, g2_se_pr) # Comparing distribution of standard errors in promoters
    ...
    row_height_factor = 0.5  # inches per row
    col_width_factor = 0.5   # inches per column
    ...
    if 'significant' in df.columns:
        ...
        for i, row_idx in enumerate(pivot_df.index):
            for j, col_idx in enumerate(pivot_df.columns):
                ...
                g.ax_heatmap.text(j + 0.5, i + 0.5, text, ha='center', va='center', color='black', fontsize="x-small")
