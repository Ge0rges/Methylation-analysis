...
def identify_predictive_features(X, y, n_top_features=10):
    """
    Identify features most predictive of binary class using:
    - Mutual information
    - Welch's t-test (absolute statistic)
    
    Returns top n features from each method, plus a consensus rank.
    """
    ...
    # 2. Welch's t-test (unpaired, unequal variance)
    ...
    t_table = pd.DataFrame({
        "feature": X.columns,
        "abs_t_stat": abs_t_stats,
        "p_value": t_pvals
    }).sort_values('abs_t_stat', ascending=False)
    
    # 3. Consensus: average rank from both methods
    mi_ranks = mi_table.reset_index(drop=True).reset_index().set_index('feature')['index']
    t_ranks = t_table.reset_index(drop=True).reset_index().set_index('feature')['index']
    avg_ranks = (mi_ranks + t_ranks) / 2
    consensus = pd.DataFrame({
        "feature": X.columns,
        "average_rank": avg_ranks
    }).sort_values('average_rank').reset_index(drop=True)
    
    # Output top n from each
    return {
        "mutual_info": mi_table.head(n_top_features),
        "ttest": t_table.head(n_top_features),
        "consensus": consensus.head(n_top_features)
    }