...
def analyze_colwellia(
    ...
    for motif in motifs:
        ...
        # reg_df = regulatory_candidates(motif, promoter=True)
        # synthesis(motif=motif, ensemble_df=ensemble_df, frac_groups_df=frac_groups_df, pca_df=pca_df, reg_df=reg_df, promoter=True)

        # # All
        # ensemble_df = ensemble_significant_features(motif, promoter=False)
        # frac_groups_df = frac_investigation_with_stats(motif, promoter=False)
        # pca_df = annotated_pca(motif, promoter=False)
        # reg_df = regulatory_candidates(motif, promoter=False)
        # synthesis(motif=motif, ensemble_df=ensemble_df, frac_groups_df=frac_groups_df, pca_df=pca_df, reg_df=reg_df, promoter=False)
