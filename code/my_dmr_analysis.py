...
def run_analysis(genome_name, data_dir, fig_savepath="plots"):
    ...
    # Load the data
    methyl_data = load_combined_methyl_data_for_genome_polars(genome_name, data_dir,
                                                              common_locations=False).collect().lazy()
    ...
    methyl_data = add_gene_caller_id(methyl_data, genes, True).select(
        pl.exclude("contig", "strand", "start", "end", "direction")).collect(streaming=True)

    # Run the Rao score
    print(add_rao_score_by_gene(methyl_data, sample_filter, baseline=False))


if __name__ == "__main__":
    # For each folder in the data directory
    data_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../data/methylation_data/methylation_5")
    folders = [f for f in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, f))]

    run_analysis("Pelagibacter_r-contigs", data_dir, fig_savepath="../plots/plots_5")
